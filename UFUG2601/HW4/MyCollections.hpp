...
private:
    ...
    LinkedListElement *start, *end;
    int length;
public:
    LinkedList() {
        start = new LinkedListElement, end = new LinkedListElement;
        start->next = end, end->prev = start;
        length = 0;
    }
    ~LinkedList() {
        for (LinkedListElement *ele = start, *nxt = ele; ele != end; ele = nxt) {
            nxt = ele->next;
            delete ele;
        }
        delete end;
    ...
    bool contains(const E& element) const override {
        for (LinkedListElement *ele = start->next; ele != end; ele = ele->next)
    ...
    void clear() {
        length = 0;
        for (LinkedListElement *ele = start->next, *nxt = ele; ele != end; ele = nxt) {
            nxt = ele->next;
            delete ele;
        }
        start->next = end, end->prev = start;
    ...
    void add(const E& element) override {
        LinkedListElement *newEle = new LinkedListElement;
    ...
    E removeIndex(int index) override {
        ...
        LinkedListElement *ele;
        E res;
        if (index >= length >> 1) for (ele = end->prev, index = length - index - 1; index; ele = ele->prev, index--) ;
