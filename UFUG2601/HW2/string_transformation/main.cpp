...
int main()
{
	...
	for (int i = 2; i <= len; i++) {
		if (str[i] == '-') {
			// the single minus should be printed
			if (i == len) putchar('-');
			else {
				// the neighbour characters
				char lst = str[i - 1], nxt = str[i + 1];
				// the neighbour characters should be number or letter
				if ((is_number(lst) && is_number(nxt)) || (is_letter(lst) && is_letter(lst))) {
					if (lst < nxt) {
						// print what the problem requires
