    ...
    \For {$(\mathrm{X}, \bf{y})\in \mathcal{C}$}
      ...
      \For {$j=1$ to $m$}
        ...
        \State {$p\leftarrow \nabla_{f_{\theta_0}(\mathrm{X}+\delta_{j,0})}\left(
          L(f_\theta(\mathrm{X}+\delta_{j,0}), \bf{y})
        \right)$}
        \For {$k=1$ to $n$}
          \State $g_a\leftarrow
        ...
        \EndFor
        \State $g_\theta\leftarrow g_\theta + \nabla_{\theta}(L(f_{\theta}(\mathrm{X}+\delta_{j,n}), \bf{y}))$
  ...
  \For {$i\leftarrow 1$ to $N$}
    ...
    \For {$(\mathrm{X}, \bf{y})\in \mathcal{C}$}
      ...
      \For {$i\leftarrow 1$ to $k$}
        \State {execute backpropagation}
        \State $g_\theta\leftarrow g_\theta + \frac{1}{k}\nabla_{\theta}(L(f_\theta(\mathrm{X}+\delta_{i-1}), \bf{y}))$
        \State $g_a\leftarrow \nabla_{\mathrm{X}+\delta}(L(f_\theta(\mathrm{X}+\delta_{i-1}), \bf{y}))$
...
\mathrm{Breg}_\mathcal{D}(\theta_1, \theta_2)=\frac{1}{\left\vert\mathcal{D}\right\vert}
  \sum_{(\bf{x}, \bf{y})\in\mathcal{D}}l\left(f_{\theta_1}(\bf{x}), f_{\theta_2}(\bf{x})\right)
$$
...
\begin{bmatrix}
  \nabla_{\delta_1}l(f_\theta(\bf{x}_1), f_\theta(\bf{x}_1+\delta_1)) \\
  \nabla_{\delta_2}l(f_\theta(\bf{x}_2), f_\theta(\bf{x}_2+\delta_2)) \\
  \vdots \\
  \nabla_{\delta_{|\mathcal{D}|}}l\left(f_\theta\left(\bf{x}_{|\mathcal{D}|}\right), f_\theta\left(\bf{x}_{|\mathcal{D}|}+\delta_{|\mathcal{D}|}\right)\right)
\end{bmatrix}
$$
...
\begin{algorithmic}[1]
  ...
  \For {$t\leftarrow 1$ to $N$}
    ...
    \For {$(\mathrm{X}, \bf{y})\in \mathcal{C}$}
      ...
      \For {$i=1$ to $k$}
        \State $g_i\leftarrow \frac{g_{\overline{\theta}}\left((\mathrm{X},\bf{y}), \delta\right)}{\left\Vert g_{\overline{\theta}}((\mathrm{X},\bf{y}), \delta) \right\Vert}$
        \State $\delta\leftarrow \mathrm{clamp}(\delta + \alpha g_i, -\epsilon, \epsilon)$
      \EndFor
      \State $\overline{\theta}\leftarrow \mathrm{Adam}\left(\overline{\theta}, (\mathrm{X}+\delta, \bf{y})\right)$
    \EndFor
